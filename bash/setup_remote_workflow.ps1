...
#   .\bash\setup_remote_workflow.ps1 -WithNgrok
#   .\bash\setup_remote_workflow.ps1 -SkipChannelLogin
#   .\bash\setup_remote_workflow.ps1 -StatusOnly
...
if ($null -eq $clawPath) {
    ...
    Write-Host "     See openclaw documentation for installation." -ForegroundColor Gray
    exit 2
}
...
$gatewayUp = $false
$attempt = 1
while ($attempt -le 6 -and -not $gatewayUp) {
    try {
        $null = Invoke-WebRequest -Uri "http://127.0.0.1:18789/health" -TimeoutSec 4 -UseBasicParsing -ErrorAction Stop
        $gatewayUp = $true
        Ok "Gateway responding (attempt $attempt)"
    } catch {
        Warn "Gateway not yet up (attempt $attempt/6)..."
...
if ($SkipChannelLogin) {
    ...
    $null = & $OpenclawBin channels status --channel telegram --json 2>&1
    if ($LASTEXITCODE -eq 0) {
        Ok "Telegram channel already linked"
    } else {
        Write-Host "  Linking Telegram bot (token in openclaw.json)..." -ForegroundColor Gray
...
$null = & $OpenclawBin channels status --channel whatsapp --json 2>&1
if ($LASTEXITCODE -eq 0) {
...
if ($WithNgrok) {
    ...
    if ($null -eq $ngrokPath) {
        ...
        Start-Sleep -Seconds 3
        try {
            $tunnels = Invoke-WebRequest -Uri "http://127.0.0.1:4040/api/tunnels" -TimeoutSec 5 -UseBasicParsing -ErrorAction Stop | ConvertFrom-Json
            $publicUrl = $tunnels.tunnels[0].public_url
            Ok "ngrok tunnel: $publicUrl"
            Write-Host "  Interactions API public: $publicUrl/interactions" -ForegroundColor Gray
            Write-Host "  Set in .env: INTERACTIONS_ENDPOINT_URL=$publicUrl/interactions" -ForegroundColor Gray
        } catch {
            Warn "Could not read ngrok URL - check http://localhost:4040"
        }
    }
} else {
