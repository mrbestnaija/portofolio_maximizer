...
**Recent changes (2025-12-08)**: Synthetic generator now defaults to Phase 1/2 event+regime+market-hours+microstructure hooks (Spread/Slippage columns; `config/synthetic_data_config.yml` carries defaults; `SYNTHETIC_DATASET_ID=latest` points to syn_682c415bb89c), GPU preference wiring added to pipeline/backtest/GAN stubs (`PIPELINE_DEVICE` auto-detects CUDA with CPU fallback), and a minimal GAN trainer/runner (`scripts/train_gan_stub.py`, `bash/run_gan_stub.sh`) consumes persisted synthetic parquet with checkpoints under `models/synthetic/gan_stub`.
**Recent Achievements**:
...
### Synthetic event & microstructure additions (2025-12-08)
- `config/synthetic_data_config.yml` now ships default Phase 1/2 events (flash crash, vol spike, gap risk, bear drift), regime/correlation toggles, market-hours seasonality weights, and microstructure knobs (spread/slippage/micro_vol) so new datasets exercise stress hooks by default.
- `scripts/generate_synthetic_dataset.py` persists the new columns (Spread, Slippage) into parquet/manifest and refreshes `data/synthetic/latest.json` so `SYNTHETIC_DATASET_ID=latest` resolves to `syn_682c415bb89c`.
- `etl/synthetic_extractor.py` reads `SYNTHETIC_DATASET_PATH`/`SYNTHETIC_DATASET_ID` and outputs Spread/Slippage for downstream feature builders, keeping the extractor/pipeline in sync with the config.
...
- Timeline: Phase 1/2 synthetic event/regime + market-hours/microstructure hooks are live; Phase 2 macro event library/crisis catalog and calibration sweeps remain in progress; GPU preference wiring is landed across pipeline/backtests/GAN stub.
- Phase status: Synthetic stack is green for default runs with Spread/Slippage outputs; GPU auto-detect defaults to CUDA when present with silent CPU fallback; GAN stub remains optional/torch-gated.
- Run IDs: `data/synthetic/latest.json` → `syn_682c415bb89c` (events+microstructure+seasonality); prior baselines `syn_fc8a37128efc` (events) and `syn_714ae868f78b` (pre-events) remain for comparison. Latest smoke ran via `bash/run_synthetic_latest.sh` using `SYNTHETIC_DATASET_ID=latest`.
