...
- On Windows, the preferred human-facing launch path is `python scripts/windows_dashboard_manager.py launch` or `.\launch_live_dashboard.bat`; this refreshes the payload, starts the local bridge/server/exporter stack, opens the dashboard, and leaves the live watcher as an opt-in flag (`--ensure-live-watcher`).
