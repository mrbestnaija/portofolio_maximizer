...
- Time Series signals: per-ticker thresholds from `config/signal_routing_config.yml` now apply (previously ignored), round-trip friction is estimated from microstructure (`TxnCostBps`/`ImpactBps`) or bid/ask spreads with symmetric net-edge (SELL no longer “benefits” from costs), CI-based uncertainty ratios are dimensionally consistent, optional signal-to-noise gating is supported via `time_series.cost_model.min_signal_to_noise`, and MSSA-RL change-point recency nudges risk/confidence.
- Quant validation: expected_profit uses net trade return and historical metrics are computed directionally for BUY vs SELL to keep the gate mathematically consistent.
- Routing: TS signals now include `risk_level` (derived from `risk_score`) for execution-layer validation/sizing heuristics.
- Execution validation: `ai_llm.signal_validator.SignalValidator` now prefers TS `provenance.decision_context` (net edge + round-trip cost) over historical drift proxies in the transaction-cost layer, and uses `forecast_horizon` as the holding-period fallback when no explicit edge is provided.
- Config wiring: `signal_routing.time_series.cost_model` now ships in `config/signal_routing_config.yml` (default_roundtrip_cost_bps + min_signal_to_noise); legacy `friction_buffers` (USD-like proposal artifacts) are removed from the runtime config.
- Portfolio impact: validator layer 4 now consumes an execution portfolio snapshot (passed from `execution.paper_trading_engine.PaperTradingEngine`) and enforces basic single-name concentration + gross exposure guards when increasing exposure.
