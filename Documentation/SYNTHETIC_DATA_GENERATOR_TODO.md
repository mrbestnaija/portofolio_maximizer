...
- **Provenance + dashboards**: ETL/auto‑trader now emit `logs/automation/db_provenance_<run>.json` and dashboards surface `run_id` + `dataset_id` badges. Any dashboard with `data_origin=synthetic` or `mixed` is explicitly **not a profitability proof artifact**—use the badge to gate promotion decisions.
- **LOB execution realism**: Depth/spread + tail‑depth profiles are configured in `config/execution_cost_model.yml` for both synthetic microstructure generation and signal gating. Keep per‑asset class depth/spread values in sync with any synthetic calibration so slippage/cost checks stay size‑aware.
