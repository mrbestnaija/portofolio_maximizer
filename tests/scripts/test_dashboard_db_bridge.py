...
def test_build_dashboard_payload_from_sqlite(tmp_path) -> None:
    ...
    try:
        ...
        fresh_position_date = datetime.now(timezone.utc).date().isoformat()
        conn.execute(
            "INSERT INTO portfolio_positions(ticker,position_date,shares,average_cost) VALUES (?,?,?,?)",
            ("AAPL", fresh_position_date, 10, 101.0),
        )
        conn.execute(
...
def test_positions_stale_uses_filtered_execution_fallback(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        conn.execute(
            "CREATE TABLE trade_executions(id INTEGER PRIMARY KEY, ticker TEXT, action TEXT, shares REAL, price REAL, trade_date TEXT, created_at TEXT, realized_pnl REAL, realized_pnl_pct REAL, mid_slippage_bps REAL, is_diagnostic INTEGER, is_synthetic INTEGER, run_id TEXT, exit_reason TEXT)"
        )
        conn.execute(
            "CREATE TABLE portfolio_positions(id INTEGER PRIMARY KEY, ticker TEXT, position_date TEXT, shares REAL, average_cost REAL)"
        )
        ...
        stale_date = (datetime.now(timezone.utc) - timedelta(days=16)).date().isoformat()
        conn.execute(
            "INSERT INTO portfolio_positions(ticker,position_date,shares,average_cost) VALUES (?,?,?,?)",
            ("AAPL", stale_date, 99, 100.0),
        )
        conn.execute(
            "INSERT INTO trade_executions(ticker,action,shares,price,trade_date,created_at,realized_pnl,realized_pnl_pct,mid_slippage_bps,is_diagnostic,is_synthetic,run_id,exit_reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("AAPL", "BUY", 10, 100.0, "2026-03-01", "2026-03-01T00:00:01Z", None, None, 5.0, 0, 0, "RID", None),
        )
        conn.execute(
            "INSERT INTO trade_executions(ticker,action,shares,price,trade_date,created_at,realized_pnl,realized_pnl_pct,mid_slippage_bps,is_diagnostic,is_synthetic,run_id,exit_reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("DIAG", "BUY", 50, 10.0, "2026-03-01", "2026-03-01T00:00:02Z", None, None, 5.0, 1, 0, "RID", None),
        )
        conn.execute(
            "INSERT INTO trade_executions(ticker,action,shares,price,trade_date,created_at,realized_pnl,realized_pnl_pct,mid_slippage_bps,is_diagnostic,is_synthetic,run_id,exit_reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("SYN", "BUY", 20, 10.0, "2026-03-01", "2026-03-01T00:00:03Z", None, None, 5.0, 0, 1, "RID", None),
        )
        conn.execute(
            "INSERT INTO performance_metrics(total_return,total_return_pct,win_rate,profit_factor,num_trades,created_at) VALUES (?,?,?,?,?,?)",
            (5.0, 0.0002, 1.0, 2.0, 1, "2026-03-02T00:00:00Z"),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    assert payload["positions"]["AAPL"]["shares"] == 10
    assert "DIAG" not in payload["positions"]
    assert "SYN" not in payload["positions"]


def test_performance_unknown_when_metrics_missing(tmp_path) -> None:
    db_path = tmp_path / "pmx_missing_perf.db"
    conn = sqlite3.connect(db_path)
    try:
        conn.execute("CREATE TABLE ohlcv_data(date TEXT, ticker TEXT, close REAL)")
        conn.execute(
...
def test_trade_events_include_exit_reason_when_available(tmp_path) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            ("AAPL", "SELL", 10, 101.0, "2026-01-02", "2026-01-02T00:00:01Z", 5.0, 0.005, 10.0, "RID", "STOP_LOSS"),
        )
        conn.execute(
            "INSERT INTO performance_metrics(total_return,total_return_pct,win_rate,profit_factor,num_trades,created_at) VALUES (?,?,?,?,?,?)",
            (5.0, 0.0002, 1.0, 2.0, 1, "2026-01-02T00:00:00Z"),
    ...
    assert payload["trade_events"][0]["exit_reason"] == "STOP_LOSS"
