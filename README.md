...
- **Capital readiness still treats definitively negative lift CI as advisory only.** `scripts/capital_readiness_check.py` warns on `R5` even when both CI bounds are below zero, while `scripts/check_model_improvement.py` already hard-fails Layer 1 for the same condition. That cross-layer mismatch is still open on `master`.
- **The live dashboard is connected, but not yet fully authoritative.** `scripts/dashboard_db_bridge.py` still trusts stale `portfolio_positions` rows without a DB freshness gate, still zero-fills unavailable performance fields, and still drops `exit_reason` from trade events in the current `master` path.
