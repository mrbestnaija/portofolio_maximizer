...
- **Capital readiness now fail-closes on definitively negative lift CI.** `scripts/capital_readiness_check.py` blocks readiness when `R5` has both CI bounds below zero; CI bands that span zero remain advisory warnings.
- **Dashboard payload now distinguishes stale and unknown states explicitly.** `scripts/dashboard_db_bridge.py` emits `positions_stale`, `positions_source`, `positions_age_days`, and `performance_unknown`; `visualizations/live_dashboard.html` renders stale warnings, `N/A` for unknown performance, and `Last Exit` from `exit_reason`.
