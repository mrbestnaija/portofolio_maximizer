...
class MSSARLConfig:
    ...
    change_point_threshold: float = 10.0
...
class MSSARLForecaster:
    ...
    def _resolve_active_action(self) -> int:
        ...
        if self._policy_status != "ready":
            if not bool(self.config.policy_fail_closed):
                logger.warning(
                    "MSSA-RL policy_fail_closed=false is deprecated; refusing live "
                    "forecast because policy_status=%s.",
                    self._policy_status,
                )
            raise ValueError(
