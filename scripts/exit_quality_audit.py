...
def load_production_trades(db_path: Path, tail_n: int | None = None) -> pd.DataFrame:
    ...
    atr_series = (df["bar_high"] - df["bar_low"]).astype(float)
    df["atr_proxy"] = np.where(has_atr, atr_series, np.nan)
    # Fallback: 1.5% of entry_price
    no_atr = df["atr_proxy"].isna() & df["entry_price"].notna() & (df["entry_price"] > 0)
    df.loc[no_atr, "atr_proxy"] = (df.loc[no_atr, "entry_price"] * 0.015).astype(float)
