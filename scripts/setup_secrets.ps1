...
Write-Host "[INFO] Setting up Docker secrets directory..." -ForegroundColor Cyan

# Create secrets directory if it doesn't exist
if (-not (Test-Path $SECRETS_DIR)) {
    New-Item -ItemType Directory -Path $SECRETS_DIR -Force | Out-Null
    Write-Host "[OK] Created secrets directory: $SECRETS_DIR" -ForegroundColor Green
} else {
    Write-Host "[OK] Secrets directory already exists: $SECRETS_DIR" -ForegroundColor Green
}
...
Write-Host "[OK] Set permissions on secrets directory" -ForegroundColor Green
...
if (-not (Test-Path $alphaFile)) {
    ...
    Write-Host "[WARN] Created placeholder file: $alphaFile" -ForegroundColor Yellow
...
if (-not (Test-Path $finnhubFile)) {
    ...
    Write-Host "[WARN] Created placeholder file: $finnhubFile" -ForegroundColor Yellow
    Write-Host "   Please update with your actual API key!" -ForegroundColor Yellow
}
...
Write-Host "[OK] Set permissions on secret files" -ForegroundColor Green

Write-Host ""
Write-Host "[OK] Secrets directory setup complete!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "[WARN] IMPORTANT: Never commit secrets to git!" -ForegroundColor Red
