...
def main():
    ...
    if quant_data and quant_data['total'] > 0:
        ...
        print("  1. Ensemble policy labels flagged (RESEARCH_ONLY/DISABLE_DEFAULT)")
    ...
    if pipeline_data:
        ...
        if blocked > 0 and approved == 0:
            print("⚠️  CRITICAL: All forecasts were flagged by the ensemble policy label")
            print()
            print("ROOT CAUSE:")
            print("  - Policy label uses promotion_margin (e.g., 2% RMSE lift) and RMSE tolerance gates")
            print("  - Current ensemble provides insufficient improvement")
            print("  - RMSE regression detected (error > 1.1x baseline)")
            print()
            print("SOLUTIONS:")
            print("  1. Review ensemble governance thresholds (promotion_margin/max_rmse_ratio/max_violation_rate)")
